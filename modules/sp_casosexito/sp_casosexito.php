...
include_once(_PS_MODULE_DIR_.'sp_casosexito/Sp_CasoExito.php');

class Sp_CasosExito extends Module implements WidgetInterface
{
    protected $_html = '';
    ...
    public function __construct()
    {
        ...
        $this->displayName = 'Casos de exito';
        $this->description = 'Permite agregar casos de exito';
        $this->ps_versions_compliancy = array('min' => '1.7.1.0', 'max' => _PS_VERSION_);

        $this->templateFile = 'module:sp_casosexito/views/templates/hook/casosexito.tpl';
    }
    ...
    public function install()
    {
	  ...
	  //       $lang = ($this->context->language->id) ? $this->context->language->id : _PS_LANG_DEFAULT_ ;
			...
			// $parent_tab->name[$lang] = 'Casos de Exito';
			// $parent_tab->class_name = 'CasosExitoMain';
			...
			// $tab = new Tab();
			// // Need a foreach for the language
			// $tab->name[$lang] = 'Casos de Exito';
			// $tab->class_name = 'Casos';
      ...
      protected function createTables()
    {
        ...
        $res = Db::getInstance()->execute('
            CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'casos_exito` (
              `id_caso_exito` int(10) unsigned NOT NULL AUTO_INCREMENT,
              `titulo` varchar(255) NOT NULL,
              `imagen_principal` varchar(255) NOT NULL,
              `imagen_destacado` varchar(255) NOT NULL,
              `descripcion` text NOT NULL,
              `posicion_imagen` tinyint(1),
    ...
    protected function deleteTables()
    {
        ...
        foreach ($sections as $section) {
            $to_del = new Sp_CasoExito($section['id_section']);
            $to_del->delete();
        }

        return Db::getInstance()->execute('
            DROP TABLE IF EXISTS `'._DB_PREFIX_.'casos_exito`;
        ');
    }
    ...
    public function getContent()
    {
        ...
        if (
            ...
            if ($mode == 'add') {
                ...
                $associated_shop_ids = Sp_CasoExito::getAssociatedIdsShop((int)Tools::getValue('id_section'));
        ...
        if(Tools::isSubmit('submitBanner')){
            ...
            if (!$res) {
             ...
             if (Tools::getValue('id_pregunta_frecuente')) {
                $pregunta = new Sp_CasoExito((int)Tools::getValue('id_pregunta_frecuente'));
            ...
            $section = new Sp_CasoExito((int)Tools::getValue('id_section'));
            if ($section->active == 0) {
                $section->active = 1;
            } else {
                $section->active = 0;
            ...
            $section = new Sp_CasoExito((int)Tools::getValue('delete_id_section'));
            $res = $section->delete();
            $this->clearCache();
            if (!$res) {
    ...
    public function getPreguntas($active = null)
    {
        ...
        // foreach ($sections as &$section) {
        //     $section['image_url'] = $this->context->link->getMediaLink(_MODULE_DIR_.'Sp_CasoExitos/images/'.$section['image']);
        //     $section['url'] = $this->updateUrl($section['url']);

        //     $section['image_title_url'] = $this->context->link->getMediaLink(_MODULE_DIR_.'Sp_CasoExitos/images/'.$section['image_title']);
        // }
